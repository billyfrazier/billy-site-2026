...
rigged) from two crops of the launch photos, then repaired — see
[tools/model/README.md](tools/model/README.md), which reproduces
`public/models/billy-body.glb` from the raw byte-for-byte. In short: the book
he was holding baked onto his tee (recolouring its yellow, the first attempt,
just left the cover's type and shading behind as a dark rectangle), the black
jeans carried a red bleed, and the generator's material was fully self-lit
(`emissiveFactor [1,1,1]` with the base colour as emissive map), so the studio
lights did nothing. Now emissive `0.28` and specular `1` (it shipped a
non-physical `2`). Texture is 2048 again at webp q72 — the figure's face is
the whole point of the page, and 1024 visibly mushes the glasses and beard.
948KB, up from 653KB.

Two scan artifacts survive, both ~1px at render scale: a hairline of cover
left between his fingertips, and the repainted area on the tee reading very
slightly flatter than the shirt around it. Both were left alone deliberately —
every attempt to chase them widened the mask until it notched the suede.

Blink is disabled for this model. The eye coordinates in blink.js were measured
against the bust, and this mesh is skinned: its geometry lives in a tiny bind
space, so re-measuring means selecting the eyes in skinned space and mapping
the squash back through the head bone's inverse bind matrix.
...
his left, `Head.z` positive tilts toward his right shoulder; `RightArm.x`
negative raises the arm out sideways (LeftArm mirrors, positive);
